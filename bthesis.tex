...
The second experiment evaluated the network performance and available bandwidth with iperf. This experiment deserves two special comments. First, as mentioned in Chapter 4, the total achievable network throughput in RFsimulator mode depends on the underlying hardware, the actual numbers are, therefore irrelevant, as they scale with the hardware. Second, in evaluating two deployments, where the OAI emulated part of the network is the same, the only factors influencing the different results are the rest of the network between UPF and cloud, distance and possibly node performance. Because of the resulting numbers (under 10 Mb/s for both edge and cloud), neither network nor node performance are expected to have a major impact. The difference between both achieved throughputs is therefore probably achieved because of the higher communication latency between UE and cloud.
\begin{table}[!ht]
    \centering
